...
// 1. Crea una función ferraris que devuelva todos los coches de la marca Ferrari.

const ferraris = function () {
    const marca = cars.filter(car => car.brand == 'Ferrari');
    console.log(marca);
}
console.log('Coches de la marca Ferrari:');
ferraris();

// 2. Cree una función ninetiesque devuelva todos los autos con modelos entre 1980 y 1990.
console.log();

const ninetiesque = function () {
    const autos = cars.filter(car => car.model >= 1980 && car.model <= 1990);
    console.log(autos);
}
console.log('Autos con modelos entre 1980 y 1990:');
ninetiesque();

// 3. Cree una función list que devuelva una matriz de cadenas con la misma cantidad de autos. 
// Cada elemento debe contener la cadena "Estedecuesta $” 
// (por ejemplo, para el primer elemento debe decir “Este Mazda de 1989 cuesta $5241”).
// “This Mazda from 1989 costs $5241”).
console.log();

const list = function () {
    const matrizCadenas = cars.map(car => 'This ' + car.brand + 
                                ' from ' + car.model + 
                                ' costs ' + car.price);
    console.log(matrizCadenas);
}
console.log('Matriz de cadenas con la misma cantidad de autos:');
list();
